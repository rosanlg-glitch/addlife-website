...
export default function ReviewSection() {
  ...
  const scrollRight = () => {
    if (active < reviews.length - visibleCount) setActive(active + 1);
  };

  const visibleReviews = reviews.slice(active, active + visibleCount);

  return (
    <div className="dark bg-surface py-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-3">
          {clientLogos.slice(0, 10).map((logo) => (
            <div
              key={logo.id}
              className="border border-gray-600 rounded-lg px-4 py-2 flex items-center justify-center"
            >
              {logo.image ? (
                <img
                  src={logo.image}
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="w-[220px] flex-shrink-0">
            ...
            <h3 className="text-white text-2xl font-bold mt-2 mb-8">
          ...
          <div className="flex gap-4 flex-1">
            {visibleReviews.map((review) => (
              <div
                key={review.id}
                className="flex-1 bg-white rounded-2xl p-6 flex flex-col justify-between"
              >
                {/* Quote */}
                <div>
        ...
        <div className="flex justify-center gap-3 mt-8">
          {reviews.map(
            (_, index) =>
              index < reviews.length - visibleCount + 1 && (
                <button
                  key={index}
